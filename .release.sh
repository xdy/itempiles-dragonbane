MANIFEST_SEARCH_PATTERN='(\s\"(manifest)\"\: \"https:\/\/github.com\/xdy\/itempiles-dragonbane\/releases\/).*(\/(module.json)\",)'
DOWNLOAD_SEARCH_PATTERN='(\s\"(download)\"\: \"https:\/\/github.com\/xdy\/itempiles-dragonbane\/releases\/).*(\/(itempiles-dragonbane.zip)\",)'
...
npm install &&
cd dist || exit &&
zip -r itempiles-dragonbane.zip ./* &&
cd ..
